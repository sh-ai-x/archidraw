...
import {getShapeText} from "./scene";

const SHAPE_LABEL_FONT = "Inter, ui-sans-serif, system-ui, sans-serif";

/**
 * Render a shape label after the rough draw call. No-op when the text
 * is missing or whitespace-only. Per-shape alignment comes from the
 * fixed table in the plan:
 *   rectangle → (x+6, y+6)         left/top
 *   diamond   → (x+w/2+6, y+h/2)   left/middle
 *   ellipse   → (x+6, y+h/2)       left/middle
 * fontSize falls back to 14 (rectangle) or 16 (diamond/ellipse).
 */
const drawShapeLabel = (ctx: CanvasRenderingContext2D, e: Element): void => {
  const text = getShapeText(e);
  if (text === "") return;
  const fontSize =
    typeof (e as any).fontSize === "number" && (e as any).fontSize > 0
      ? (e as any).fontSize
      : e.type === "rectangle"
        ? 14
        : 16;
  ctx.fillStyle = e.strokeColor;
  ctx.font = `${fontSize}px ${SHAPE_LABEL_FONT}`;
  let tx = e.x;
  let ty = e.y;
  if (e.type === "rectangle") {
    ctx.textAlign = "left";
...
export const renderScene=(canvas:HTMLCanvasElement,elements:Element[],zoom=1,pan={x:0,y:0},selected:string|null=null,marquee?:{x1:number;y1:number;x2:number;y2:number}|null,multiSel:string[]=[])=>{const ctx=canvas.getContext("2d");if(!ctx)return;ctx.clearRect(0,0,canvas.width,canvas.height);ctx.save();ctx.scale(devicePixelRatio,devicePixelRatio);ctx.translate(pan.x,pan.y);ctx.scale(zoom,zoom);const rc=rough.canvas(canvas);for(const e of elements){if(e.isDeleted)continue;const opts={seed:e.seed,stroke:e.strokeColor,strokeWidth:e.strokeWidth,roughness:e.roughness,fillStyle:e.fillStyle||"solid",fill:e.backgroundColor==="transparent"?undefined:e.backgroundColor};if(e.type==="rectangle"||e.type==="diamond"){const points=e.type==="diamond"?[[e.x+e.width/2,e.y],[e.x+e.width,e.y+e.height/2],[e.x+e.width/2,e.y+e.height],[e.x,e.y+e.height/2],[e.x+e.width/2,e.y]]:[[e.x,e.y],[e.x+e.width,e.y],[e.x+e.width,e.y+e.height],[e.x,e.y+e.height],[e.x,e.y]];rc.path(`M ${points.map(p=>p.join(" ")).join(" L ")}`,opts);drawShapeLabel(ctx,e)}else if(e.type==="ellipse"){rc.ellipse(e.x+e.width/2,e.y+e.height/2,Math.abs(e.width),Math.abs(e.height),opts);drawShapeLabel(ctx,e)}else if(e.type==="line"||e.type==="arrow"){
        ...
        }else if(e.type==="text"){ctx.fillStyle=e.strokeColor;ctx.font=`${e.fontSize || 12}px ${SHAPE_LABEL_FONT}`;const _ta=e.textAlign||"start";const _va=e.verticalAlign||"top";ctx.textAlign=_ta;ctx.textBaseline=_va==="middle"?"middle":_va==="bottom"?"bottom":"top";let _tx=e.x;if(_ta==="center")_tx=e.x+(e.width||0)/2;else if(_ta==="right")_tx=e.x+(e.width||0);let _ty=e.y;if(_va==="middle")_ty=e.y+(e.height||0)/2;else if(_va==="bottom")_ty=e.y+(e.height||0);else _ty=e.y+(e.fontSize||14)*0.9;ctx.fillText(e.text||"",_tx,_ty)}if(selected===e.id){
